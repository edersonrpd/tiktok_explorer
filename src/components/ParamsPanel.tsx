 ...
 * nada além do que a documentação prevê.
 *
...
export function ParamsPanel({
  ...
  const optionalCount = params.filter(
    (p) => allowedSet.has(p.name) && !requiredSet.has(p.name),
  ).length;
  const complete = presentRequired === expected.length && extras.length === 0;
        ...
        <dl className="space-y-0.5">
          {params.map((p, i) => (
            <div key={`${p.name}-${i}`} className="flex gap-2 font-mono text-[11px]">
              <dt
                className={
                  allowedSet.has(p.name) ? "shrink-0 font-bold t-2" : "shrink-0 font-bold text-amber-700"
                }
              >
                {p.name}
